...
import type { NavigationItem } from '@/data/homepage-data';

const Logo = memo(function Logo() {
  return <span className="text-2xl font-bold text-green-600">WealthSetu</span>;
});
...
const MobileNavigationLink = memo(function MobileNavigationLink({
  item,
}: {
  item: NavigationItem;
}) {
...
export default function Header({ className = '' }: HeaderProps) {
  ...
  useEffect(() => {
    async function loadNavigation() {
      try {
        const nav = await HomepageDataService.getNavigation();
